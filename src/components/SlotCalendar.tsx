...
export default function SlotCalendar({ activeVenue, onVenueChange, days, activeDay, onDayChange, slots, onToggleSlot }:
  { activeVenue: string; onVenueChange: (id: string) => void; days: Date[]; activeDay: number; onDayChange: (i: number) => void; slots: Record<string, SlotStatus>; onToggleSlot: (hour: number) => void }) {
      ...
      <div className="bg-[#16291C] border border-[#1E3324] rounded-lg p-6">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          ...
          <div className="flex gap-4 text-xs">
            {[["bg-[#16291C] border border-[#2C4A33]","Open"],["bg-[#8BC34A]","Booked"],["bg-[#E5484D]/30 border border-[#E5484D]/50","Blocked"]].map(([color,label])=>(
