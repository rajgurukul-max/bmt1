...
export default function Page() {
  ...
  const stats = useMemo(() => {
    let bookedCount = 0, total = 0;
    Object.values(slots).forEach((s) => { total++; if (s === "booked") bookedCount++; });
    return {
      occupancy: total ? Math.round((bookedCount / total) * 100) : 0,
      todayRevenue: BOOKINGS.filter((b) => b.date === "Today" && b.status === "confirmed").reduce((a, b) => a + b.amount, 0),
          ...
          {tab === "dashboard" && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                <StatCard icon={IndianRupee} label="Today's revenue" value={`₹${stats.todayRevenue.toLocaleString("en-IN")}`} />
              ...
              <div className="bg-[#16291C] border border-[#1E3324] rounded-lg p-6">
                <h2 className="text-sm font-medium text-[#9FB0A3] mb-4 uppercase tracking-wide">Recent bookings</h2>
                <BookingList bookings={BOOKINGS.slice(0, 3)} />
              </div>
            </>
          )}
          {tab === "calendar" && (
            <SlotCalendar activeVenue={activeVenue} onVenueChange={setActiveVenue} days={DAYS} activeDay={activeDay} onDayChange={setActiveDay} slots={slots} onToggleSlot={toggleSlot} />
          )}
          {tab === "bookings" && (
            <div className="bg-[#16291C] border border-[#1E3324] rounded-lg p-6">
